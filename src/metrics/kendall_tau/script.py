...
def compute_kendall_tau(true_values, inferred_values):
    """Calculates Kendall tau rank correlation on finite overlapping values."""
    mask = np.isfinite(true_values) & np.isfinite(inferred_values)
    
    if mask.sum() < 2:
        return 0.0
    kendall_tau, _ = stats.kendalltau(true_values[mask], inferred_values[mask])
...
score = compute_kendall_tau(true_vals, inferred_vals)


# metric_ids and metric_values can have length > 1
# but should be of equal length
uns_metric_ids = [ 'kendall_tau' ]
